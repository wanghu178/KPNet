...
class GenerationModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if torch.is_tensor(output_netG):
            ...
            self.aux_sdr = output_netG[1]        
            sdr1_loss = self.cri_pix(self.aux_sdr[0],self.var_L)
            sdr2_loss = self.cri_pix(self.aux_sdr[1],self.var_L)
            sdr3_loss = self.cri_pix(self.aux_sdr[2],self.var_L)
            total_loss = sdr1_loss + sdr2_loss + sdr3_loss
            l_pix =  self.cri_pix(self.fake_H,self.real_H) + self.l_pix_w*total_loss
        l_pix.backward()
