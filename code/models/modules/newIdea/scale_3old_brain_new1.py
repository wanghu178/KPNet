...
class KUNet2(nn.Module):
    def __init__(self,in_nc=3, out_nc=3, nf=64, act_type='relu'):
        ...
        self.nam_att = wh_util.CoordAtt(nf,nf)
    ...
    def forward(self, x):
        ...
        KIB_2 = self.recon_trunk(fea2)*self.alpha(fea1_down) + self.belta(fea1_down)
        
        out = self.act(self.up_conv1(KIB_2))
        out = self.nam_att(out) 
        KIB_3 = self.recon_trunk(out+fea1)*self.alpha(fea1) + self.belta(fea1)
