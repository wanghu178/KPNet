...
class KUNet2(nn.Module):
    def __init__(self,in_nc=3, out_nc=3, nf=64, act_type='relu'):
        ...
        self.brain = wh_util.brain_new1(nf)
    ...
    def forward(self, x):
        ...
        KIB_1 = self.recon_trunk1(fea_kib1)*self.alpha(fea_kib1) + self.belta(fea_kib1)
        
        fea1_down = self.act(self.down_conv2(fea1))
        fea_kib2 = self.dislocation_change2(fea1_down)
        KIB_2 = self.recon_trunk2(fea_kib2)*self.alpha(fea_kib2) + self.belta(fea_kib2)
        
        fea_kib3 = self.dislocation_change3(fea0)
        KIB_3 = self.recon_trunk3(fea_kib3)*self.alpha(fea_kib3) + self.belta(fea_kib3)

        brain_process = self.brain(KIB_1,self.up_KIB2(KIB_2),self.down_conv3(KIB_3))
